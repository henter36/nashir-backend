...
function getTestDatabaseUrl(): string {
  if (!testDatabaseUrl) {
    throw new Error(
      "TEST_DATABASE_URL is required for idempotency repository tests"
    );
  }
  ...
  if (!databaseName || !databaseName.includes("test")) {
    throw new Error(
      `Refusing to use non-test database "${databaseName || "unknown"}" in idempotency repository tests`
    );
  }
...
async function truncateIdempotencyData(pool: pg.Pool) {
  await pool.query(
