...
5. `main` is clean and `npm run lint`, `npm run typecheck`, `npm run format:check`, and `npm test` all pass on the commit the execution gate would branch from.
...
| Pre-existing formatting drift risk | `npm run format:check` continues to report Prettier warnings on three files (`scripts/validate-contract-authority.mjs`, `scripts/validate-contracts.mjs`, `src/request-context.ts`) that predate PR #12, as documented by PR #13 | Confirmed unchanged by re-running `npm run format:check` on current `main`; this does not block this decision, but Section 8 precondition 5 requires `format:check` to pass on the commit any future execution gate branches from — meaning that gate's own scope must include resolving this formatting before it may merge any change touching those files |
